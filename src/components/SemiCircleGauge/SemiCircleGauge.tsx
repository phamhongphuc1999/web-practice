...
export default function GradientSemiCircleGauge({ size = 100, percent, ...props }: Props) {
  const { y1, y2 } = useMemo(() => {
    const _alpha = (1 - percent * percent * 0.3) * Math.PI;
  ...
  return (
    ...
    >
      <svg
        ...
        style={{
          filter: 'drop-shadow(-10px 10px 15px rgba(175, 74, 243, 0.4))',
          overflow: 'visible',
        }}
      >
        <mask id="semiMask">
        ...
        <path d={path} fill="white" fillOpacity="0.04" />
        <defs>
          <linearGradient
            id="semi_gauge"
            x1="0%"
