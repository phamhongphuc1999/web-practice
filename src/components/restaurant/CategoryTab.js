...
const CategoryTag = () => {
  ...
  return (
    <div className={cx("grid grid-cols-3", styles.restaurantTabContainer)}>
      <div className={cx("col-span-1", styles.tabElement, {
        "bg-white": display[0] || tab === 0,
        "bg-transparent": !display[0]
      })}
        onMouseEnter={() => onTabEnter(0)} onMouseLeave={() => onTabLeave()}
        onClick={() => onTabClick(0)}>
        <div className={cx(styles.tabIcon, "bg-yellow-700")}>
          <BiRestaurant size="40" color="white" />
        </div>
        <span className={styles.tabTitle}>{t('restaurantPage.tab.foodAndDrink')}</span>
      ...
      <div className={cx("col-span-1", styles.tabElement, {
        ...
        onMouseEnter={() => onTabEnter(1)} onMouseLeave={() => onTabLeave()}
        onClick={() => onTabClick(1)}>
        <div className={cx(styles.tabIcon, "bg-red-300")}>
          <GiCakeSlice size="40" color="white" />
        </div>
        <span className={styles.tabTitle}>{t('restaurantPage.tab.dessert')}</span>
        <div className={cx(styles.list, {
      ...
      <div className={cx("col-span-1", styles.tabElement, {
        ...
        onMouseEnter={() => onTabEnter(2)} onMouseLeave={() => onTabLeave()}
        onClick={() => onTabClick(2)}>
        <div className={cx(styles.tabIcon, "bg-green-400")}>
          <BiHappyBeaming size="40" color="white" />
        </div>
        <span className={styles.tabTitle}>{t('restaurantPage.tab.entertainment')}</span>
        <div className={cx(styles.list, {
